...
echo "define channel(PEER1_INT400) chltype(sdr) conname(10.230.168.10) xmitq(INT400.PEER1.TQ) trptype(tcp)"|/opt/mqm/bin/runmqsc
...
echo "DEFINE QLOCAL(INT400.PEER1.TQ) DESCR('Transmission queue to live') USAGE(XMITQ)"|/opt/mqm/bin/runmqsc PEER1_INT400

echo "start channel (PEER1_INT400)"|/opt/mqm/bin/runmqsc PEER1_INT400
echo "DEFINE QLOCAL (CAT.IN)"|/opt/mqm/bin/runmqsc PEER1_INT400

echo "DEFINE QREMOTE(IPSCNC.OUT) RNAME(IPSCNC.IN) RQMNAME(INT400) XMITQ(INT400.PEER1.TQ)"|/opt/mqm/bin/runmqsc PEER1_INT400

echo "DEFINE QREMOTE(IPSQ.OUT) RNAME(IPSQ.IN) RQMNAME(INT400) XMITQ(INT400.PEER1.TQ)"|/opt/mqm/bin/runmqsc PEER1_INT400

echo "DEFINE QREMOTE(REDPROD.OUT) RNAME(REDPROD.IN) RQMNAME(INT400) XMITQ(INT400.PEER1.TQ)"|/opt/mqm/bin/runmqsc PEER1_INT400

echo "DEFINE QREMOTE(WAT.ORDEBK.OUT) RNAME(WAT.ORDEBK.IN) RQMNAME(INT400) XMITQ(INT400.PEER1.TQ)"|/opt/mqm/bin/runmqsc PEER1_INT400
